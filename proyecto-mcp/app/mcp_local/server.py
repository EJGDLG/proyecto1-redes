...
def cyclomatic_complexity(node: ast.AST) -> int:
    # Ciclomática básica: +1 por cada bifurcación/loop/except/and/or/comprehension if
    walkers = (
        ast.If, ast.For, ast.While, ast.And, ast.Or, ast.ExceptHandler,
        ast.With, ast.BoolOp, ast.Try, ast.IfExp, ast.Match, ast.Case
    )
    count = 1
    for n in ast.walk(node):
        if isinstance(n, (ast.If, ast.For, ast.While, ast.With, ast.Try, ast.IfExp, ast.Match)):
            count += 1
        elif isinstance(n, ast.BoolOp):  # and/or con n valores = n-1 bifurcaciones
